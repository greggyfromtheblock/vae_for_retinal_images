...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="""The path to the image dir"""
    )
    parser.add_argument(
        "csv_input",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="""subdir inside path_prefix dir where the file is to be saved"""
    )
    parser.add_argument(
        "path_prefix",
        type=str,
        default=None,
        help="""path_prefix for the output dir file to be saved"""
    )
...
#    print("FLAGS= ", FLAGS)
    ...
    imfolder = add_slash(os.path.abspath(args.input))
    csv_file = os.path.abspath(args.csv_input)
    #latent_vector_size = FLAGS.zdim
    latent_vector_size = 3*256*320 #the resized image size

#    network_name = FLAGS.network_name
    path_prefix = add_slash(args.path_prefix)
#    network_dir = f'{path_prefix}/{network_name}/'
    network_dir = add_slash(args.output)
    output = add_slash(args.output)

    print("\nLoad Data as Tensors...")
    img_dataset = datasets.ImageFolder(
        imfolder,
        transform=transforms.Compose(
            [transforms.ToTensor(), transforms.transforms.torch.flatten]
...
#    encoded_samples = features.detach().numpy()
    ...
    for i, diagnosis in tqdm(enumerate(diagnoses.keys())):
        ...
        if diagnosis_name == "normal fundus":
            ...
            plt.savefig(
                f"{path_prefix + output}visualizations/tsne_visualization_of_diagnosis_{diagnosis_name}.png")
            #plt.show()
            plt.close()
...
#                f"{path_prefix}/{network_name}/visualizations/umap_visualization_of_diagnosis_{diagnosis_name}.png")
                f"{path_prefix + output}visualizations/umap_visualization_of_diagnosis_{diagnosis_name}.png")
            #plt.show()
            plt.close()
...
#            plt.savefig(f"{path_prefix}/{network_name}/visualizations/umap_visualization_of_diagnosis_{diagnosis_name}.png")
            plt.savefig(f"{path_prefix + output}visualizations/umap_visualization_of_diagnosis_{diagnosis_name}.png")
            #plt.show()
            plt.close()
