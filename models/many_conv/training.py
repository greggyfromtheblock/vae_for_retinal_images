dd the Training (TorchSupport-Training API) and loss functions here.
"""
import torch
...
class OdirVAETraining(VAETraining):
    ...
    def run_networks(self, data, *args):
        ...
        for i in range(0, 50, 10):
            reconstructions[i] = F.sigmoid(reconstructions[i])
        if self.step_id % 20 == 0:
            self.writer.add_images("target", data[0:50:10], self.step_id)
            self.writer.add_images("reconstruction", reconstructions[0], self.step_id)
